...
export const Projects = () => {
  ...
  return (
    <div>
      <Typography variant="h6" align="center" gutterBottom style={{ marginBottom: '20px', color: '#666' }}>Add your Mini Projects</Typography>
      {projects.map((item, index) => (
        <div key={item.id} className={`${classes.instance} ${fallbackInstanceClasses}`}>
          <Grid container spacing={3}>
            <Grid item xs={12} sm={6}>
              <TextField fullWidth variant="outlined" label="Project Name*" name="projectName" value={item.projectName || ''} onChange={(e) => handleChange(index, 'projectName', e.target.value)} inputProps={{ name: 'projectName' }} />
            </Grid>
            <Grid item xs={12} sm={6}>
              <TextField fullWidth variant="outlined" label="Tech Stack" name="techStack" value={item.techStack || ''} onChange={(e) => handleChange(index, 'techStack', e.target.value)} inputProps={{ name: 'techStack' }} />
            </Grid>
            <Grid item xs={12}>
              <TextField fullWidth variant="outlined" label="Description" name="description" multiline rows={3} value={item.description || ''} onChange={(e) => handleChange(index, 'description', e.target.value)} inputProps={{ name: 'description' }} />
            </Grid>
          </Grid>
